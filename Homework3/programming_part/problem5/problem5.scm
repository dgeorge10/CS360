...
(define fcs-data-base
  '(
(rule (answer ?g)
	  (and (csg PH100 ?s ?g) (snap ?s (L. Van Pelt) ?a ?p)))

(rule (before ?x ?y)
	  (cp ?x ?y))
(rule (before ?x ?y)
	  (and (cp ?x ?z) (before ?z ?y)))
...
;; Databases required for Problem 5
;; Load all databases in (not in query driver loop) as follows:
;; (initialize-data-base fcs-data-base) 
;; Then run the query driver loop:
;; (query-driver-loop)
;; To answer the query in 14.3.1:
;; (answer ?g)
;; To prove the facts in 14.9.2:
;; (before CS120 CS100)
;; (before CS206 CS100)
